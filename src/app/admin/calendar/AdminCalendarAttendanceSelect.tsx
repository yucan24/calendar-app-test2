...
type AdminCalendarAttendanceSelectProps = {
  eventId: string;
  userId: string;
...
export default function AdminCalendarAttendanceSelect({
  eventId,
  userId,
  defaultStatus,
}: AdminCalendarAttendanceSelectProps) {
  ...
  function handleChange(nextStatus: string) {
    ...
    startTransition(async () => {
      try {
        await updateAdminCalendarAttendance(eventId, userId, nextStatus);
        router.refresh();
      } catch {
